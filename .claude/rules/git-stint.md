...
### Project override (IVSBox / T113_TCP)

This repo is shared by **multiple AI agents** operating the same working copy.
`AGENTS.md` rule 8 mandates an ownership prefix so each agent's session is
identifiable. Use these instead of the generic names above:

- WorkBuddy uses `wb-<topic>` (e.g. `wb-proto`)
- TRAE uses `trae-<topic>` (e.g. `trae-netmgr`)

Also, always pass `--session <name>` explicitly on every `git stint`
sub-command: on Windows this tool reports `Multiple active sessions` even when
the shell is already inside a worktree.
