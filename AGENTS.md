...
1. **文档唯一性**：所有 AI 一律使用本仓库中已存在的权威文档，不得各自维护平行副本：
   ...
   - 开发计划：`ivsbox-v5/docs/功能开发计划.md`（2026-09-24 经用户决定替代退役原《开发流程.md》）
   - 架构与缺口：根目录两份架构文档
...
| 功能开发计划 | `ivsbox-v5/docs/功能开发计划.md` | M1~M8 全阶段详细计划：每步"实现什么/怎么实现/怎么验证"三要素＋§0.1 实现状态总表；2026-09-24 经用户决定新建并**替代退役**《开发流程.md》（后者已删除，历史见 git） |
...
| 日常开发闭环流程 | `ivsbox-v5/docs/操作手册/日常开发闭环流程.md` | SSH 编译 VM → 交叉编译 → SCP 上板 → COM4 串口调试验证的通用链路，S01~S20 每条任务都走；经用户确认新建（规则 7 第 2 条） |
