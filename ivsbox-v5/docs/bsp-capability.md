...
### 7.6 待安装库清单（自研替代中，做镜像时统一加入）

> 规则来源：开发列表 §6 纪律第 11 条（用户定，2026-09-24）——依赖选型：**优先现成 Linux 库 → 没有 → 先自研保进度 → 登记到本表 → 做镜像时加入并替换自研代码**。
> 登记模板：| 库名 | 用途 / 替代的自研代码 | 需要的开发包（sysroot） | 首次登记 | 备注 |

| 库名 | 用途 / 替代的自研代码 | 需要的开发包（sysroot） | 首次登记 | 备注 |
|---|---|---|---|---|
| （暂无条目——出现首个"两侧都没有、只能自研"的依赖时在此登记） | | | | |
...
| 功能 | 状态 | 完成时间 | 说明 |
|---|---|---|---|
| §7.6 待安装库清单（依赖选型纪律第 11 条的登记处） | 已实现 | 2026-09-24 11:35 | 用户定规则：优先现成库 → 自研保进度 → 登记本表 → 做镜像时加入并替换 |
